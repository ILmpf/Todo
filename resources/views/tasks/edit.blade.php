<x-layouts.app title="Editar Tarefa">
    ...
    <nav class="flex items-center gap-2 text-sm text-warm-500 mb-6" aria-label="Breadcrumb">
        ...
        <a href="{{ route('tasks.show', $task) }}" class="hover:text-accent-600 transition-colors truncate max-w-48">
            {{ $task->title }}
        </a>
        <svg class="w-4 h-4 text-warm-300" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
        </svg>
        <span class="text-warm-700 font-medium">Editar</span>
    </nav>
    ...
    <div class="max-w-2xl">
        ...
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-warm-900">Editar Tarefa</h1>
            <p class="mt-1 text-sm text-warm-500">Atualiza os detalhes da tarefa abaixo.</p>
        </div>

        <!-- Form Card -->
        <div class="bg-beige-50 rounded-2xl border border-beige-200 shadow-sm overflow-hidden">

            <form id="edit-form" method="POST" action="{{ route('tasks.update', $task) }}" class="divide-y divide-beige-200">
                @csrf
                @method('PUT')
                ...
                <div class="px-6 py-5">
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        class="w-full rounded-lg border px-3 py-2.5 text-sm text-warm-900 placeholder-warm-400 resize-none transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent
                            {{ $errors->has('description') ? 'border-red-300 bg-red-50' : 'border-beige-200 bg-beige-50' }}">{{ old('description', $task->description) }}</textarea>
                    @error('description')
                ...
                <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
                    ...
                    <!-- Status -->
                    <div>
                        <label for="status" class="block text-sm font-semibold text-warm-700 mb-1.5">
                            Estado <span class="text-red-500">*</span>
                        </label>
                        <select id="status" name="status"
                            class="w-full rounded-lg border px-3 py-2.5 text-sm text-warm-900 bg-beige-50 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent
                                {{ $errors->has('status') ? 'border-red-300' : 'border-beige-200' }}">
            ...
            <div class="px-6 py-4 bg-beige-100 border-t border-beige-200 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
                ...
                <form method="POST" action="{{ route('tasks.destroy', $task) }}"
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="w-full sm:w-auto text-center text-sm font-medium text-red-500 hover:text-red-700 hover:bg-red-50 px-4 py-2.5 rounded-lg border border-transparent hover:border-red-200 transition-colors">
                        Eliminar Tarefa
                    </button>
                </form>
